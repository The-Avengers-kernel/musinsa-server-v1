        ...
        <div class="product-right">
            ...
            <div class="options-section">
                ...
                <div class="total-price-box">
                    <span>총 상품 금액</span>
                    <span class="total-price" id="finalTotalPriceDisplay"></span>
                </div>
            </div>
    ...
    <div class="product-description-area">
        ...
        <div class="product-specs">
            ...
            <div class="detail-group">
                <strong>COLOR:</strong>
                <div id="productOptionColor" class="detail-list"></div>
            </div>

            <div class="detail-group">
                <strong>FABRIC/MATERIAL:</strong>
                <div id="productMaterial" class="detail-list"></div>
            </div>

            <div class="detail-group">
                <strong>SIZE:</strong>
                <div id="productOptionSize" class="detail-list"></div>
            </div>
    ...
    $(document).ready(function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.brandImage) {
                    const $brandImageContainer = $('#brand-image-container').empty(); // 새로운 ID 사용
                    const imgTag = $('<img>')
                        .attr('src', data.brandImage)
                        .attr('alt', data.brandName)
                        .addClass('brand-logo-img'); // CSS 스타일 적용을 위해 클래스 추가
                ...
                $('#finalTotalPriceDisplay').text(data.finalprice.toLocaleString() + '원'); // 총 상품 금액에도 적용
                ...
                if (data && data.detailSizeImage) {
                    const imgTag = $('<img>')
                        ...
                        .attr('alt', '제품 실측 이미지')
                        .addClass('detail-size-img'); // 필요시 CSS 클래스

                    $('#detailSizeImage').empty().append(imgTag);
                } else {
                    $('#detailSizeImage').text('이미지를 불러올 수 없습니다.');
                }

                // 이미지 렌더링 (메인 이미지와 썸네일 구분)
                ...
                if (data.productImageList && data.productImageList.length) {
                    data.productImageList.forEach(function (img) {
                        ...
                        if (img.imageType === 'MAIN') {
                            $mainImageContainer.append(imgTag.clone()); // 메인 영역에 추가
                        } else {
                            // 썸네일 미리보기 영역에 추가
                ...
                $('.add-to-cart-btn').off('click').on('click', function () {
                    alert(data.productName + '이(가) 장바구니에 담겼습니다.');
                    // 실제 장바구니 로직 구현
                });
            },
    ...
    $(document).ready(function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.productOptionColor && data.productOptionColor.length){
                    data.productOptionColor.forEach(function (value){
                        const $color = $('<option>').val(value).text(value);
                ...
                if(data.productOptionMaterial && data.productOptionMaterial.length){
                    data.productOptionMaterial.forEach(function (value){
                        const $material = $('<option>').val(value).text(value);
                ...
                if(data.productOptionSize && data.productOptionSize.length){
                    data.productOptionSize.forEach(function (value){
            ...
            error: function (xhr, status, error) {
                console.error('제품 옵션 정보 로드 오류:', error);
            }
        });
    });
</script>
