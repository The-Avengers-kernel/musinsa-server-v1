...
<div class="page">
    ...
    <div class="card">
        ...
        <div class="field">
            ...
            <input type="text" id="recipientName" class="input" maxlength="20" placeholder="받는 분의 이름을 입력해주세요">
        </div>

        <!-- 휴대폰번호 -->
        <div class="field">
            <div class="label">휴대폰번호</div>
            <input type="tel" id="recipientPhone" class="input" maxlength="13" placeholder="휴대폰번호를 입력해주세요. 예) 010-1234-5678">
        </div>
        ...
        <div class="field">
            ...
            <div class="zip-row" style="margin-bottom:8px;">
                <input type="text" id="postalCode" class="input" inputmode="numeric" maxlength="5" placeholder="우편번호">
                <button type="button" class="btn" id="btnFindAddr">주소 찾기</button>
            </div>

            <input type="text" id="address1" class="input" placeholder="주소" style="margin-bottom:8px;">
            <input type="text" id="address2" class="input" maxlength="100" placeholder="상세주소">
...
<script>
    ...
    $("#saveBtn").on("click", function(){
        ...
        const fullAddr = $("#address1").val().trim() + " " + $("#address2").val().trim();
        $("#recipientAddress").val(fullAddr);

        const requestMsg = ($("#request").val()==="직접입력") ? $("#requestManual").val().trim() : $("#request").val();

        const data = {
            userId: Number("${userId}"), // TODO: 세션에서 로그인 사용자 ID 주입
        ...
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/api/v1/add",
            contentType: "application/json; charset=UTF-8",
            data: JSON.stringify(data),
            success: function(){
                alert("배송지가 저장되었습니다!");
                window.location.href = "${pageContext.request.contextPath}/shipping-address-popup";
            },
            error: function(xhr){
                alert("저장 실패: " + (xhr.responseText || "알 수 없는 오류"));
            }
        });
    });
</script>
