...
class AtomicUpdateVsPessimisticLockTest {
    ...
    @Test
    @DisplayName("원자적 UPDATE vs 비관적 락 성능 비교")
    ...
    private void printComparison(long atomicTime, long pessimisticTime) {
        ...
        double timeDiff = (double) pessimisticTime / atomicTime;

        System.out.println("원자적 UPDATE:  " + atomicTime + " ms");
        System.out.println("비관적 락:      " + pessimisticTime + " ms");
        System.out.println("\n차이:           " + (pessimisticTime - atomicTime) + " ms");
        System.out.println("배수:           비관적 락이 " + String.format("%.2f", timeDiff) + "배 느림");
        System.out.println("결론: 원자적 UPDATE가 " + String.format("%.1f", timeDiff) + "배 빠름!");
        System.out.println("=".repeat(80) + "\n");
    }
}